 ...
 * @LastEditors: gxn
 * @LastEditTime: 2022-05-08 09:33:13
 * @Description: 组织机构管理钩子函数
 */
...
export default function ({
  ...
  const [internalSearchSelectTreeValue, setInternalSearchSelectTreeValue] = useState('');
    ...
    if (modalType === 'edit') {
      ...
      if (targetType === DICT_TYPES.class.key) {
        ...
        const {
          dicttypeId,
          dictdataParentid
        } = selectedNode; // 根字典没有dictdataParentid
      ...
      // 字典下创建字典
      if (targetType === DICT_TYPES.class.key && selectedNodeType === DICT_TYPES.dict.key) {
        const {
          dicttypeId
        } = selectedNode;
