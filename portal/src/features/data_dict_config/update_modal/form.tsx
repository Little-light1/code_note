...
import {useModal, PageContext, useAction} from '@gwaapp/ease';
...
interface CustomFormProps {
    ...
    targetTypes2: CreateAbleTypes[]; // 要添加的对象
...
const DataDictForm = ({
    ...
    const {id} = pageProps;
    const [targetTypes, setTargetTypes] = useState<any[]>(targetTypes2 || []);
    ...
    const selectableTree = useMemo(
        () => loopSelectableTree(dictTypeTree, currentType, selectedNode),
        [dictTypeTree, currentType, selectedNode],
    ); // 初始化内部父子关系
    ...
    }, [initRelationship, modalType, selectedNodeType, selectedNode]); // 初始化选择状态
    ...
    useEffect(() => {
        ...
        if (modalType === 'edit') {
            ...
            if (selectedNode.type === DICT_TYPES.class.key) {
                fields = {
                    ...fields,
                    dictdataSort: 1,
                };
        ...
        <div className={`${styles.view}`}>
            <Form
                ...
                onValuesChange={(changedValues: {[key: string]: any}) => {
                    ...
                    if (changedValues.hasOwnProperty('parentId')) {
                        const selectNewNode =
                            dictTypeMap[changedValues.parentId];
                        if (modalType === 'edit') {
                            if (selectNewNode.modifiable) {
                                setTargetTypes([selectNewNode?.type]);
                            }
                        } else if (modalType === 'add') {
                            if (selectNewNode.addable) {
                                setTargetTypes(selectNewNode.addable);
                            }
                        }
                    }
                }}
