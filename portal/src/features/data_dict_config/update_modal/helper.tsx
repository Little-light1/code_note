...
export function loopSelectableTree(
    ...
    return treeData.map((item: any) => {
        const {children, type, hasDataChild, addable} = item;
        const newItem = {...item};

        if (selectedNode && item.dicttypeCode === selectedNode.dicttypeCode) {
        ...
        if (currentType === DICT_TYPES.class.key) {
            ...
            if (!isClass || haveDictInChildren || hasDataChild === 1) {
                // 循环设置子为不可选中
                if (newItem.children) {
                    newItem.children = loopSelectableTree(
                        children,
            ...
            if (
                haveChildrenOrChildrenAllDict ||
                (addable &&
                    addable.length === 1 &&
                    addable[0] === DICT_TYPES.dict.key)
            ) {
                newItem.disabled = false;
            } else {
                newItem.disabled = true;
